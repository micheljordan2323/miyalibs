...
def measure():
  ...
  read_meas_result = read_meas_interval(h)
  ...
  # if not every 2s, set it
    ...
    ret = i2cWrite(h,[0x46, 0x00, 0x00, 0x02, 0xE3])
    if ret == -1:
      exit(1)
    read_meas_interval()
  ...
  # print "CRC: " + hex(crc8)
  i2cWrite(h,[0x00, 0x10, pressure[0], pressure[1], crc8])

  # read ready status
  while True:
    ret = i2cWrite(h,[0x02, 0x02])
    if ret == -1:
      exit(1)

    data = read_n_bytes(h,3)
    if data == False:
      time.sleep(0.1)
      continue
  ...
  #read measurement
  i2cWrite(h,[0x03, 0x00])
  data = read_n_bytes(h,18)
  ...
  pi.i2c_close(h)
  return float_co2,float_rH,float_T

def read_n_bytes(h,n):
...
def read_meas_interval(h):
  ret = i2cWrite(h,[0x46, 0x00])
  if ret == -1:
    return -1
